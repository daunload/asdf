...
test.describe('Magic Link Authentication', () => {
	test('should authenticate user via magic link', async ({
		page,
		context,
	}) => {
...
Cypress.Commands.add('authViaMagicLink', (email) => {
	return dataSession({
		...
		setup: () => {
			...
			cy.get('[data-cy="user-menu"]', { timeout: 10000 }).should(
				'be.visible',
			);
		...
		recreate: (cached) => {
			...
			cy.visit('/dashboard');
			cy.get('[data-cy="user-menu"]', { timeout: 5000 }).should(
				'be.visible',
			);
		},
...
test.describe('Email Auth Negative Flows', () => {
	...
	test('should rate-limit excessive magic link requests', async ({
		page,
	}) => {
		const randomId = Math.floor(Math.random() * 1000000);
		const testEmail = `user-${randomId}@${MAILOSAUR_SERVER_ID}.mailosaur.net`;
		...
		for (let i = 0; i < 10; i++) {
			...
			if (errorVisible) {
				...
				await expect(
					page.getByTestId('rate-limit-error'),
