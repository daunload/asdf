...
export async function generateCard(
    ...
    try {
            ...
            if (error.status === 429) {
                ...
                if (retryCount < maxRetries) {
                    ...
                    const retryAfter = error.headers?.['retry-after']
                        ? parseInt(error.headers['retry-after'] as string, 10) * 1000
                ...
                // 최대 재시도 횟수 초과
                throw new Error('LLM API 호출 한도가 초과되었습니다. 잠시 후 다시 시도해주세요.');
            } else if (error.status === 500 || error.status === 503) {
                // 서버 오류 - 재시도 가능
                if (retryCount < maxRetries) {
                    const retryDelay = Math.min(1000 * Math.pow(2, retryCount), 5000); // 최대 5초
                    console.log(
                ...
                throw new Error('LLM 서비스에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해주세요.');
        ...
        if (error.message?.includes('timeout') || error.message?.includes('TIMEOUT')) {
            ...
            throw new Error('LLM API 호출이 시간 초과되었습니다. 다시 시도해주세요.');
        }
    ...
    for (let i = 0; i < requests.length; i++) {
        ...
        try {
            ...
            results.push({
                ...
                card: {
                    symbol: '⚠️',
                    body: error.message || '카드 생성 중 오류가 발생했습니다. 다시 시도해주세요.',
                    cta: '다시 시도',
                },
            });
        }
